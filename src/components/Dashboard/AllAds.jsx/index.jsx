            ...
            <table className="w-full min-w-[500px] text-left rtl:text-right">
                <thead className="bg-[#20C5F2]">
                    <tr className="flex w-full gap-2 justify-between">
                        {["#", "mainImage", "title", "price", "owners", "category","subcategory", "Action","Edit"].map((title, i) => (
                            <th key={i} scope="col" className={`flex items-center justify-center py-3 font-medium w-${i === 0 ? '14' : i === 1 ? '28' : i === 2 ? '32' : i === 3 ? '48' : i === 4 ? '52' : i === 5 ? '40' : '32'}`}>
                                {title}
                            </th>
                        ))}
                    </tr>
                ...
                <tbody>
                    {ads?.map((adsData, i) => <AdsData key={adsData.id} {...adsData} {...{deleteAds}} {...{editads}} {...{handleOption,allow, setAllow}} index={i} />)}
                </tbody>
            </table>
        </div>
